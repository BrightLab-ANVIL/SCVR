...
# lagmap=os.path.join(basedir,"grpLevel-BH18/", subj+"_lag/lagMap.nii.gz")
# out=os.path.join(basedir, "grpLevel-BH18/", subj+"_lag")

# grouplag.lagCorrectedSCVR(out,minus5,minus4,minus3,minus2,minus1,noDelay,
...
for run in range(1,19):
    ...
    groupmask=os.path.join(basedir, "grpLevel-BH18/RANDOMISE_blur_noDelay/group_mask.nii.gz")
    lagmap=os.path.join(basedir,"grpLevel-BH18/", subj+"_lag/lagMap.nii.gz")
    out=os.path.join(basedir, "grpLevel-BH18/", subj+"_lag", "copes_indiv")

    grouplag.lagCorrectedSCVR(out,minus5,minus4,minus3,minus2,minus1,noDelay,
                        plus1,plus2,plus3,plus4,plus5,maskpath=groupmask,
                        lagmappath=lagmap,prefix=prefix)